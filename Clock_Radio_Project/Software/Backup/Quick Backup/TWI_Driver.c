...
void I2C_WriteRegister_16bit(uint16_t data){
	...
	//printf("Wrote low byte: %i\n", low_byte);
	uint8_t high_byte = data >> 8;
	//printf("Wrote high byte: %i\n", high_byte);
	I2C_Write(high_byte);
	_delay_ms(1);
	I2C_Write(low_byte);
}
...
uint8_t I2C_ReadRegister(uint8_t device_address, uint8_t deviceRegister){
	...
	//printf("sent register address\n");
	I2C_Start();		//set_up-->TWINT, TWSTA, TWEN
	//printf("i2c started again\n");
	I2C_SendAddress(device_address+READ);	//Send Master RTC Addr with Ack, and Read Bit (0xd1)
	//printf("Sent address with read bit\n");
...
uint16_t I2C_ReadRegister_16bit(){
	...
	_delay_ms(1);
	uint8_t low_byte = I2C_ReadACK();	//Master Requests byte with Nack(0x58)
	while(!TW_READY);
	//printf("Read High Byte: %i\n", high_byte);
	//printf("Read Low Byte: %i\n", low_byte);
	data = high_byte << 8;
	data |= low_byte;
	return data;	//Read from SLave's info from TWI Ack Status Code
}
